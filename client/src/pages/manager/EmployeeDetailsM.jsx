...
const EmployeeDetailsM = () => {
  ...
  useEffect(() => {
    ...
    const fetchEmployee = async () => {
      try {
        const response = await api.get("/manager/get-employees");
    ...
    const fetchGoals = async () => {
      try {
        const response = await api.get(`/goal/manager/goals/${id}`);
  ...
  const handleGoalSubmit = async (e) => {
    ...
    try {
      ...
      // Refresh goals
      const goalsResponse = await api.get(`/goal/manager/goals/${id}`);
  ...
  const handleApproveGoal = async (goalId) => {
    try {
      await api.put(`/goal/${goalId}/approve`, {});
